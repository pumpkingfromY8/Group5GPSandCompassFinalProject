...
// Function to move the marker when the map is long-pressed (tap and hold)
function moveMarkerOnLongPress(event) {
    var latitude = event.latlng.lat;
    var longitude = event.latlng.lng;
    marker.setLatLng([latitude, longitude]);
    // Display new marker location coordinates
...
// Add event listener for long-pressing the map (tap and hold)
map.on('mousedown', function(event) {
    // Detect long press (tap and hold) for mobile devices
    var timeout;
    timeout = setTimeout(function() {
        moveMarkerOnLongPress(event);
    }, 500); // Adjust the duration of the long press as needed (in milliseconds)

    // Cancel long press if the touch is released before the timeout
    map.on('mouseup', function() {
        clearTimeout(timeout);
    });
});

// Prevent the default context menu from appearing on long-press
